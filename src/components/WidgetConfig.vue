        ...
        <el-form label-position="top">
            ...
            <el-form-item label="字号" v-if="Object.keys(data.options).indexOf('fontsize')>=0">
                <el-select v-model="data.options.fontsize">
                    ...
                    <el-option value="58" label="58"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="颜色" v-if="Object.keys(data.options).indexOf('fontcolor')>=0">
                <el-color-picker
            ...
            <el-form-item label="水平排列方式" v-if="Object.keys(data.options).indexOf('fontalign')>=0">
                <el-select v-model="data.options.fontalign">
            ...
            <el-form-item label="选项" v-if="data.widgetType==2 || data.widgetType==7 || data.widgetType==9">
                <el-radio-group v-model="data.options.remote" size="mini" style="margin-bottom:10px;">
                    <el-radio-button :label="true" v-if="data.widgetType==2 || data.widgetType==9">远端数据
                    </el-radio-button>
                    <el-radio-button :label="false" v-if="data.widgetType==2 || data.widgetType==7">静态数据
                    </el-radio-button>
                </el-radio-group>
                <template v-if="data.options.remote">
                    <div>
                        <el-select v-model="data.options.props.id" size="mini">
                            <el-option v-for="(item, index) in selectKeyData" :key="index" :value="item.value"
                                       :label="item.label"></el-option>
                        </el-select>
                ...
                <template v-else>
                    <template v-if="data.widgetType==2 || data.widgetType==7">
            ...
            <template v-if="Object.keys(data).indexOf('widgetType')>=0">
...
<script>
    ...
    export default {
        ...
        methods: {
            getCommonDicData: function () {
                let that = this
                request({
                    method: 'get',
                    /*headers: {'Content-Type': 'application/json; charset=UTF-8'},*/
                    url: '/formdata/getCommonDicData/0'

                })
                    .then(function (response) {
                        if (response.flag == 200) {
                            //alert(JSON.stringify(response.data))
                            that.selectKeyData = response.data
            ...
            getTableCols: function () {
                ...
                if (workflowId) {
                    request({
                        method: 'get',
                        url: '/formdata/getTableCols/' + workflowId
                    })
                        .then(function (response) {
                            if (response.flag == 200) {
                                that.keyData = response.data;
            ...
            handleKeyChooseFnc: function (val) {
                let obj = {};
                obj = this.keyData.find((item) => {
                    return item.value === val;
                });
                this.data.name = obj.label
                let placeholderTitle = '请输入'
                if (this.data.widgetType != 1) {
                    placeholderTitle = '请选择'
                }
                // alert(this.data.options.placeholder)
                //if(this.data.options.placeholder){
                //  alert(placeholderTitle+obj.label)
                this.data.options.placeholder = placeholderTitle + obj.label
                //  }
            },
